...
**v0.6.0 — Phase 5 shipped.** Closes the loop: a `TailoredResume` from the Phase 4 agent now becomes a real Google Doc + PDF. The renderer copies your master template (the master is sacred — never written to), then for each known section (summary, skills, work history, education, certifications) it re-reads the doc for fresh structural indices, finds the heading by case-insensitive alias match, and issues a per-section `documents.batchUpdate` (delete the existing content under the heading, insert the tailored content). Per-section atomicity means a botched section can't trash the whole doc. Final doc is exported to PDF and returned alongside a `RenderResult` with diffs per section. The end-to-end pipeline (URL → tailored Google Doc + PDF) now works programmatically; the Phase 6 API will wrap it behind an HTTP endpoint with SSE for live progress, and Phase 7 a React UI on top. **358 tests, 100% line + branch coverage, CI green.** See `_private/BUILD_PLAN.md` (gitignored — interview material) for the full plan.
